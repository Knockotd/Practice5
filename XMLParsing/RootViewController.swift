...
class RootViewController: UITableViewController, XMLParserDelegate {
    ...
    //파싱에 필요한 변수
    //1)태그 하나의 하나의 값을 저장할 변수
    var elementValue:String!
    //2)Book 1개를 저장할 변수
    var book:Book!
    //3)전체 데이터를 저장할 변수
    var books=[Book]()
    // MARK: - When first start comming method
    ...
    // MARK: - XMLParer Delegate
    //여는 태그를 만났을 때 호출되는 메소드
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]){
        //하나의 객체 여는 태그를 만났을 때 객체를 생성. 만약 태그 들 중에서 속성을 가진 태그가 있으면 그 속성을 찾아와서 저장
        if elementName == "Book"{
            book = Book()
    ...
    //여는 태그와 닫는 태그 사이에 작성하는 내용은 하나의 패킷 이상이 될 수 있어서 입니다.
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let imageName = "next.png"
        let image : UIImageView = UIImageView.init(image: UIImage.init(named: imageName))
