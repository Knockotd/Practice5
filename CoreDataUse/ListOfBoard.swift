...
class ListOfBoard: UITableViewController {

    //Board Entity의 모든 데이터를 저장할 프로퍼티
    lazy var list : [NSManagedObject] = {
        return self.fetch()
    }()
    //데이터를 읽어서 리턴하는 메소드
    func fetch() -> [NSManagedObject]{
        //저장소를 사용하기 위해서(저장소는 AppDelegate에 존재) AppDelegate 에 대한 포인터를 생성
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        //저장소 가져오기
        let context = appDelegate.persistentContainer.viewContext
        //'Board Entity'에서 데이터를 가져오는 객체를 생성
    ...
    func save(title:String, content:String) -> Bool{
        ...
        //로그를 추가하는 코드 -- 데이터를 저장하기 전에 작성해야 한다.
        //데이터를 삽입하기 위한 객체 생성
    ...
    func edit(object : NSManagedObject, title:String, content:String)->Bool{
        ...
        //로그를 추가하는 코드 - save 하기 전에 적어야 하는 코드
        //데이터를 삽입하기 위한 객체 생성
        let log = NSEntityDescription.insertNewObject(forEntityName: "Log", into: context) as! LogMO
        //데이터 설정
        log.regdate = Date()
    ...
    @objc func add(_ sender:Any){
        ...
        alert.addAction(UIAlertAction(title: "확인", style: .default){(_) in
            ...
            guard let title = alert.textFields?.first?.text, let content = alert.textFields?.last?.text else{
                return
            }
            //nil이 아니면 데이터 삽입
    ...
    */
    ...
    //테이블 뷰에서 edit button을 누르고 나오는 버튼을 눌렀을 때
    //호출되는 메소드입니다.
